...
不要把证书原件、客户文件、订单、签名、二维码或未脱敏记录放进本网站仓库；Vercel 可能把仓库内文件作为公开静态资源提供。请把原件放在访问受限的外部 Drive/私有目录，只向 `evidence-manifest.csv` 登记受控路径，并注明「可公开脱敏」「仅销售私发」或「禁止外发」。
...
收到后我会：在受限位置检查原件 → 脱敏 → 取得书面公开批准 → 只把批准后的公开版本放入网站 → 写入 `evidence-manifest.csv` → 挂到 Evidence 页 → 更新 sitemap。原件不得复制进本仓库。
