...
- Production deployment `dpl_F2LbdrnvUe4PsDg7Utzha5TBkXw1` reached `READY` and was aliased to `https://yujihealth.com`. Read-only production checks confirmed analytics `enabled=false`, aligned Privacy copy, the frontend inquiry-reference behavior, HTTP 405 plus `X-RFQ-ID` for GET, and HTTP 400 plus `X-RFQ-ID` for an empty invalid POST. No real email was sent during this verification.
- A fresh post-release GSC query for 2026-08-08 through 2026-08-11 again failed after three bounded attempts with `ETIMEDOUT`; no new search-effect claim was made.
